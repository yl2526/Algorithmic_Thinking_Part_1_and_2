...
from format_helper import int_with_commas
...
def closed_main(data, color_dict, category, duration, annotate_dict, 
    ...
    if save:                    
        fig.savefig(ax.get_title(), dpi = 400, facecolor='w', edgecolor='w', bbox_inches='tight')
        plt.close(fig)
        return ax.get_title()
    else:
...
def closed_main_split(data, color_dict, category, duration, annotate_dict, unit_median = 'Hours',
    ...
    if save:
        left_fig.savefig(left_name + '.png', dpi = 400, facecolor='w', edgecolor='w', bbox_inches='tight')
        plt.close(left_fig)
        right_fig.savefig(right_name + '.png', dpi = 400, facecolor='w', edgecolor='w', bbox_inches='tight')
        plt.close(right_fig)        
...
def closed_weekly(file_list, color_dict, category, duration, sla = 48, sla_unit = 'Hours',
    ...
    if save:                    
        plt.savefig(ax.get_title(), dpi = 400, facecolor='w', edgecolor='w', bbox_inches='tight')
        plt.close(fig)
        return ax.get_title()
    else:
        return ax.get_title(), fig, ax
